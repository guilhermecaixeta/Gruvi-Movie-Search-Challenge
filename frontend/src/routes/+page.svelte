<script lang="ts">
  ...
  async function getMoviesData() {
	...
	const response: MovieSearchResponse = await fetch(`/api/movies?${params.toString()}`)
			.then((res) => res.json())
			.then((data) => data as MovieSearchResponse);
...
<main class="p-8 max-w-6xl mx-auto">
	...
	<form on:submit|preventDefault={handleSubmit} class="mb-8 p-6 bg-white shadow-md rounded-lg">
					...
					{#each paginatedData as item (item?.id)}
						<tr class="hover:bg-gray-50">
							...
							<td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"
								>{item.title}</td
							>
							<td class="px-6 py-4 text-sm text-gray-500 text-left text-wrap">{item.description}</td
							>
			...
			<div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
				<div class="flex items-center text-sm text-gray-700">
					<p>
						...
						<span class="font-medium">{Math.min(endIdx, paginatedData.length)}</span>
